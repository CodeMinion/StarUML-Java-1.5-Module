...
begin
  ...
  for I := 0 to GetIntegerTaggedValue(AAttribute, PROFILE_JAVA, TAGSET_DIMEN, TAG_DIMEN) - 1 do
    S := S + '[]';
  // (to do) collection
...
begin
  ...
  for I := 0 to GetIntegerTaggedValue(AAssocEnd, PROFILE_JAVA, TAGSET_DIMEN, TAG_DIMEN) - 1 do
    S := S + '[]';
  // (to do) collection
  Result := S;
end;
...
begin
  ...
  for I := 0 to GetIntegerTaggedValue(AParameter, PROFILE_JAVA, TAGSET_DIMEN, TAG_DIMEN) - 1 do
    S := S + '[]';
  // (to do) collection
  Result := S;
end;
...
begin
  ...
  for I := 0 to AOperation.GetParameterCount - 1 do begin
    ...
    if Param.DirectionKind <> pdkReturn then begin
      if S <> '' then
        S := S + ', ';
      if GetBooleanTaggedValue(Param, PROFILE_JAVA, TAGSET_PARAM, TAG_FINAL) then
        S := S + 'final ';
...
begin
  ...
  if AAttribute.Changeability = ckFrozen then
    Writer.Write('final ');
  if GetBooleanTaggedValue(AAttribute, PROFILE_JAVA, TAGSET_FIELD, TAG_TRANSIENT) then
    Writer.Write('transient ');
  if GetBooleanTaggedValue(AAttribute, PROFILE_JAVA, TAGSET_FIELD, TAG_VOLATILE) then
...
begin
  ...
  if AAssocEnd.Changeability = ckFrozen then
    Writer.Write('final ');
  if GetBooleanTaggedValue(AAssocEnd, PROFILE_JAVA, TAGSET_FIELD, TAG_TRANSIENT) then
    Writer.Write('transient ');
  if GetBooleanTaggedValue(AAssocEnd, PROFILE_JAVA, TAGSET_FIELD, TAG_VOLATILE) then
    Writer.Write('volatile ');
...
begin
  ...
  if GetBooleanTaggedValue(AOperation, PROFILE_JAVA, TAGSET_OPERATION, TAG_NATIVE) then
    Writer.Write('native ');
  if GetBooleanTaggedValue(AOperation, PROFILE_JAVA, TAGSET_STRICTFP, TAG_STRICTFP) then
  ...
  Throws := GetStringTaggedValue(AOperation, PROFILE_JAVA, TAGSET_OPERATION, TAG_THROWS);
  if Throws <> '' then
...
begin
  ...
  if GetBooleanTaggedValue(AClass, PROFILE_JAVA, TAGSET_CLASS, TAG_STATIC) then
    Writer.Write('static ');
  if AClass.IsAbstract then
    Writer.Write('abstract ');
  if AClass.IsLeaf then
    Writer.Write('final ');
  if GetBooleanTaggedValue(AClass, PROFILE_JAVA, TAGSET_STRICTFP, TAG_STRICTFP) then
    Writer.Write('strictfp ');
...
begin
  ...
  if GetBooleanTaggedValue(AInterface, PROFILE_JAVA, TAGSET_STRICTFP, TAG_STRICTFP) then
    Writer.Write('strictfp ');
